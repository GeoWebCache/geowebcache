...
class DeleteManager implements Closeable {
        ...
        try {
            ...
            for (Map.Entry<Object, Object> e : deletes.entrySet()) {
                ...
                log.info(String.format(
                        "Restarting pending bulk delete on '%s/%s':%d", client.getContainerName(), prefix, timestamp));
    ...
    public class PrefixTimeBulkDelete implements Callable<Long> {
        ...
        public Long call() throws Exception {
            ...
            try {
                ...
                Predicate<BlobItem> filter = blobItem -> {
                    long lastModified = blobItem.properties().lastModified().toEpochSecond() * 1000;
                ...
                while (response.body().segment() != null) {
                    ...
                    // fetch next page
                    response = container
                            .listBlobsFlatSegment(marker, options, null)
                            .blockingGet();
                }
        ...
        private long deleteItems(ContainerURL container, BlobFlatListSegment segment, Predicate<BlobItem> filter)
                throws ExecutionException, InterruptedException {
            List<Future<Object>> collect = segment.blobItems().stream()
            ...
            try {
                ...
                if (e.response().statusCode() != NOT_FOUND.value()) {
                    throw new RuntimeException(
                            "Deletion failed with status " + e.response().statusCode() + " on resource " + key, e);
                }
            }
        }
    }
    ...
    public class KeysBulkDelete implements Callable<Long> {
        ...
        public Long call() throws Exception {
            ...
            log.info(String.format("Finished bulk delete on %s, %d objects deleted", client.getContainerName(), count));
            return count;
        }

        private long deleteItems(ContainerURL container, List<String> itemNames)
                throws ExecutionException, InterruptedException {
            List<Future<Object>> collect = itemNames.stream()
                    .map(item -> deleteExecutor.submit(() -> {
        ...
        private Object deleteItem(ContainerURL container, String item) {
            try {
                int status =
                        container.createBlobURL(item).delete().blockingGet().statusCode();
                if (status != NOT_FOUND.value() && !HttpStatus.valueOf(status).is2xxSuccessful()) {
                ...
                if (e.response().statusCode() != NOT_FOUND.value()) {
                    throw new RuntimeException(
                            "Deletion failed with status " + e.response().statusCode() + " on resource " + item, e);
                }
            }
            return null;
        }
