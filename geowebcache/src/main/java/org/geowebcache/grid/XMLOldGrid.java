...
public class XMLOldGrid {
    ...
    public GridSubset convertToGridSubset(GridSetBroker gridSetBroker) {
        ...
        if(srs.equals(SRS.getEPSG4326()) && gridBounds.equals(BoundingBox.WORLD4326) && resolutions == null) {
            ...
            if(resolutions != null) {
                gridSet = GridSetFactory.createGridSet(srs.toString(), srs, gridBounds, false, resolutions, null, null, 256, 256);
            } else {
                if(zoomStop == null) {
                    zoomStop = 30;
                }
                
                gridSet = GridSetFactory.createGridSet(srs.toString(), srs, gridBounds, false, zoomStop + 1, 256, 256);
            }
        }
