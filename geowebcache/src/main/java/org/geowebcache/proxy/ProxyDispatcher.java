...
public class ProxyDispatcher extends AbstractController {
    private static Log log = LogFactory
            .getLog(org.geowebcache.proxy.ProxyDispatcher.class);
    ...
    protected ModelAndView handleRequestInternal(HttpServletRequest request,
        ...
        if (!methStr.equalsIgnoreCase("POST")) {
            throw new ServletException("Illegal method " + methStr
                    + " for this proxy.");
        }
        ...
        synchronized (this) {
            ...
            if (time - lastRequest < 1000) {
                throw new ServletException(
                        "Only one request per second please.");
            } else {
                lastRequest = time;
            }
        }
        
        log.info("Proxying request for " + request.getRemoteAddr() + " to "
                + " " + urlStr);
