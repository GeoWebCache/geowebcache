...
public class MemoryLockProviderTest {
        ...
        return exec.submit(() -> {
            ...
            try {
                ...
                try {
                    ...
                    fail(threadNameForMessages + ": failed to release lock: " + e.getMessage());
                }
            }
        });
    }
    ...
    private static Future<?> submitLockAcquirer(
        ...
        return exec.submit(() -> {
            ...
            } catch (InterruptedException | GeoWebCacheException e) {
                Thread.currentThread().interrupt();
                fail(threadNameForMessages + " interrupted");
            }
        });
    ...
    @Test
    public void testAcquireReleaseRemovesEntry() throws GeoWebCacheException {
        MemoryLockProvider provider = new MemoryLockProvider();
        assertTrue(provider.lockAndCounters.isEmpty());

        LockProvider.Lock lock = provider.getLock("k");
        assertNotNull(lock);
    ...
    @Test
    public void testReleaseIsIdempotent() throws GeoWebCacheException {
        MemoryLockProvider provider = new MemoryLockProvider();
    ...
    @Test
    public void testNestedLocksSameThread() throws GeoWebCacheException {
        MemoryLockProvider provider = new MemoryLockProvider();

        // Nested (re-entrant) acquires on the same key
        LockProvider.Lock l1 = provider.getLock("k");
        LockProvider.Lock l2 = provider.getLock("k");
        LockProvider.Lock l3 = provider.getLock("k");
