...
public class MetastoreRemoverTest extends TestCase {
    ...
    public void testMigration(boolean migrateCreationDates) throws Exception {
        ...
        if(migrateCreationDates) {
            assertTrue(Math.abs(transformed1.lastModified() - 1348596068000d) < 1000);
        } else {
        ...
        if(migrateCreationDates) {
        	assertTrue(Math.abs(transformed2.lastModified() - 1348595993000l) < 1000);
        } else {
            assertEquals(FIXED_DATE, transformed1.lastModified());
        }
        ...
        if(migrateCreationDates) {
        	assertTrue(Math.abs(untransformed.lastModified() - 1348595928000l) < 1000);
        } else {
            assertEquals(FIXED_DATE, transformed1.lastModified());
        }
    }
