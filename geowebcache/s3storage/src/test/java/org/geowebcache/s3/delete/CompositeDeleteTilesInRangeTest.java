...
public class CompositeDeleteTilesInRangeTest {
    ...
    public void test_constructor_singleZoom_singleBound() {
        CompositeDeleteTilesInRange deleteTilesInRange = new CompositeDeleteTilesInRange(
                PREFIX, BUCKET, LAYER_ID, FORMAT_IN_KEY, SINGLE_ZOOM_SINGLE_BOUND_MATCHING);
        assertThat(
                "With a single bound in a single zoom level",
                deleteTilesInRange.children().size(),
                is(1));
        Optional<DeleteTileRange> possibleDeleteTileRange =
                deleteTilesInRange.children().stream().findFirst();
        possibleDeleteTileRange.ifPresent(deleteTileRange -> {
            assertThat(
                    "Should be a DeleteTileZoomInBoundedBox",
                    deleteTileRange,
                    is(instanceOf(DeleteTileZoomInBoundedBox.class)));
            ...
            assertThat(
                    "Child should have its grid set id set",
            ...
            assertThat(
                    "Child should have its tileRange set",
                    deleteTileZoomInBoundedBox.getTileRange(),
                    is(SINGLE_ZOOM_SINGLE_BOUND_MATCHING));
            assertThat(
                    "Child should have its zoom level set",
                    deleteTileZoomInBoundedBox.getZoomLevel(),
                    is(ZOOM_LEVEL_4));
        });
    }
}
