...
public class StatisticsTest {
    ...
    public void testAddStat() {
        ...
        assertThat(
                "Expected there to be 1 subStat", 1, is(statistics.getSubStats().size()));
        ...
        assertThat(
                "Expected recoverable issues to be set", subStats.recoverableIssues, is(statistics.recoverableIssues));
        assertThat(
                "Expected non recoverable issues to be set",
                subStats.nonRecoverableIssues,
                is(statistics.nonRecoverableIssues));
        assertThat("Expected unknown issues to be set", subStats.unknownIssues, is(statistics.unknownIssues));
        assertThat(
                "Expected the substats to be saved",
                subStats,
    ...
    public void testAddRecoverableIssue() {
        ...
        assertThat(
                ...
                statistics.getRecoverableIssues().findFirst().isPresent());
        assertThat(
                "Expected the issue to be set",
                issue,
                is(statistics.getRecoverableIssues().findFirst().get()));
    }
    ...
    public void testAddNonRecoverableIssue() {
        ...
        assertThat(
                "Expected the first issue to be present",
                statistics.getNonRecoverableIssues().findFirst().isPresent());
        assertThat(
                "Expected the issue to be set",
                issue,
                is(statistics.getNonRecoverableIssues().findFirst().get()));
    }
    ...
    public void testAddUnknownIssue() {
        ...
        assertThat(
                "Expected the first issue to be present",
                statistics.getUnknownIssues().findFirst().isPresent());
        assertThat(
                "Expected the issue to be set",
                issue,
                is(statistics.getUnknownIssues().findFirst().get()));
    }
}
