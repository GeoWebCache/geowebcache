...
Once the :ref:`Java Servlet environment <prerequisites>` is in place, installing GeoWebCache is simple.  You can download the latest Web ARchive (WAR) file from `geowebcache.org <http://geowebcache.org>`_.  Unpack the zip file and make sure to read the software :ref:`license`.
...
The file :file:`geowebcache.war` is just a zip file.  The Tomcat Administration Tool unpacks this file to a folder inside the Tomcat webapps directory called ``<tomcat dir>/webapps/geowebcache``.  If you wish, you can unpack this archive manually to this location.  You can also make configuration changes before copying to the webapps directory.
