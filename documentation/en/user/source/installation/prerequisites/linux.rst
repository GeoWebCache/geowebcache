...
* Package manager may recommend or udpate to an unsupported Apache Tomcat version.
...
Make sure you have a Java Runtime Environment (JRE) installed on your system. GeoWebCache requires a Java 17 environment, and is also tested with Java 21, available from `OpenJDK <https://openjdk.java.net/>`__, `Adoptium <https://adoptium.net/>`_, or provided by your OS distribution.
...
Open ``/opt/apache-tomcat-11.0.6/conf/tomcat-users.xml`` in a text editor. Immediately after the line containing ``<tomcat-users>``, insert::
...
``/opt/apache-tomcat-11.0.6/bin/startup.sh`` and ``/opt/apache-tomcat-a.b.c/bin/shutdown.sh``
