...
.. note:: MBTiles blob store is not activated by default. The line that includes MBTiles blob store configuration in Spring context file ``geowebcache-servlet.xml`` is commented. To activate this store uncomment that line.
