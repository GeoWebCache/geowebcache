...
The tile system as saved by GeoWebCache is "pyramidal", meaning that increasing zoom levels have a larger amount of tiles.  By default, and by far the most common case, for increasing values of z, the amount of tiles in that zoom level is four times the amount of the previous zoom level::

  [number of tiles in zoom level z] = 4 * [number of tiles in zoom level (z-1)]

When applied to square map projections where zoom level z = 0 would result in a single tile (such as the Google projection EPSG:900913 / EPSG:3857)::

  [number of tiles in zoom level z] = 4^z

When applied to rectangular map projections where zoom level z=0 would result in two tiles (such as EPSG:4326 / lat/lon)::
